...
namespace Ont3010_Project_YA2024.Controllers.Admin
{
    ...
    public class EmployeeController : BaseController
    {
        ...
        public EmployeeController(BusinessService businessService, ApplicationDbContext context, UserManager<IdentityUser> userManager, RoleManager<IdentityRole> roleManager,
                                  IWebHostEnvironment webHostEnvironment, IEmailSender emailSender, INotificationService notificationService)
                ...
                try
                {
                    ...
                    if (result.Succeeded)
                    {
                        try
                        {
                            ...
                            // Step 2: Get the EmployeeId of the newly created employee
                            var brendEmployee = await _context.Employees
                                .FirstOrDefaultAsync(e => e.Email == newEmployee.Email);

                            // Step 3: Get the ActionBy (the person who created the employee)
                            ...
                            // Step 4: Create a notification after employee is created
                            ...
                            using (var memoryStream = new MemoryStream(businessLogoBytes))
                            {
                                ...
                                message.From = new MailAddress(setting.ContactEmail, businessName); // Set your "from" email
                                ...
                                // Create HTML view and attach the image
                                ...
                                // Add priority and other headers to improve deliverability
                                message.Headers.Add("X-Priority", "1 (Highest)");
                                message.Headers.Add("X-Mailer", "MyBusinessMailer");
                                message.Headers.Add("X-Sender", setting.ContactEmail);  // Sender Email

                                // SPF and DKIM headers are set up on your domain, no need to modify code for that
                                // Just ensure your domain DNS has SPF/DKIM records

                                // Send the email
                                await _emailSender.SendEmailAsync(employee.Email, message.Subject, htmlBody);
                            }
                        ...
                        {
                            TempData["Error"] = $"Error sending welcome email: {ex.Message}";
                        }
                    }
                    else
                    {
